...
const portfolioData = [
  {
    title: 'daisyUI',
    description: 'Full-stack CRUD 게시판 개인 프로젝트',
    skills: ['HTML5', 'CSS3', 'JavaScript', 'React', 'Tailwindcss', 'Daisyui', 'Node.js', 'Express', 'PostgreSQL'],
    overview: '직원들에 대한 정보(이름, 이메일, 직업, 시급, 상태)를 담는 게시판입니다.',
    details: `
    ...
    github: 'https://github.com/jay0425/full-stack-crud',
  },
  {
    title: 'YUMMY',
    image: yummy,
    ...
    details: `
                  - 메인페이지의 추천 카페 카드 클릭 또는 지도상의 마커 클릭 시 디테일 페이지로 이동하는 기능을 구현하였습니다.<br /><br />
                  - 디테일 페이지에서는 선택한 카페의 정보와 함께 관련 블로그 글을 확인할 수 있도록 Kakao 검색 Open API를 활용하여 데이터를 불러왔습니다.<br /><br />
                  - 블로그 결과는 부드러운 흐름과 데이터 요청 최적화를 위해 react-infinite-scroller 라이브러리를 사용해 커서 기반의 무한 스크롤 방식으로 구현하였으며 마지막 페이지 도달 시 안내 메시지를 노출시켰습니다.<br /><br />
                  - 디테일 페이지 UI를 반응형으로 제작하여 데스크탑과 모바일 디스플레이에서도 최적화된 사용자 경험을 제공했습니다.<br />`,
    comment:
      '현재 토큰 만료로 인하여 메인페이지를 확인할 수 없게 되었습니다. 검색창에 케이크같은 디저트 이름이나 지역 이름을 검색하시고 뜨는 지도에서 왼쪽 상단의 화살표를 클릭하시고 뜨는 카페 리스트를 클릭하시면 디테일 페이지를 보실 수 있습니다.',
  ...
  },
  {
    ...
    details: `
              ...
              - 메인 페이지 UI에서 티빙에서 꼭 봐야하는 콘텐츠 UI를 담당했습니다. <br /><br />
              - 메인 페이지 UI에서 Quick VOD 섹션 UI를 담당했습니다.<br /><br />
              - 메인 페이지 상단 우측 아이콘을 클릭하여 나오는 로그인 창의 하단 아이디 찾기를 클릭하시면 나오는 아이디 찾기 페이지 UI를 담당했습니다.`,
  ...
  },
  {
    ...
    details: `
              - TMDB(Open Movie Database) API를 사용하여 영화 데이터를 동적으로 받아와 최신 영화, 인기 영화, 장르별 영화 정보를 제공합니다. API 호출 시 React Query를 활용하여 데이터의 캐싱 및 상태 관리를 효율적으로 처리하였습니다. <br /><br />
              - 사용자가 원하는 영화를 검색할 수 있도록 영화 검색 기능을 구현하였으며 장르별로 영화 리스트를 필터링 할 수 있는 기능도 제공하였습니다. <br /><br />
              - 영화 리스트를 swiper 컴포넌트를 활용하여 슬라이드 형식으로 구현하여 직관적인 인터페이스를 제공하고 사용자 경험을 향상시켰습니다.  <br /><br />
  ...
  },
  {
    ...
    details: `
              - 반응형 웹사이트로 제작하여 모바일, 데스크탑 같이 다양한 화면 크기에서 최적화된 UI/UX를 제공합니다. <br /><br />
              - React Router를 사용하여 페이지 전환을 구현하였습니다. 동적 라우팅을 통해 각 카테고리와 제품 상세 페이지로 원활하게 이동할 수 있도록 했습니다.<br /><br />
              - 리덕스를 통해 유동적인 데이터 사용을 구현했습니다. <br /><br />
  ...
  },
  {
    ...
    details: `- 빈 칸에 할 일을 입력하고 '할일 추가하기' 버튼을 클릭하면 할 일이 목록에 추가되도록 구현하였습니다. 입력값에 대한 유효성 검사를 추가하여 입력 없이 추가를 할 수 없게 하였고 사용자가 알기 쉽게 오류 메시지를 제공하였습니다.<br /><br />
              - 완료된 할일은 체크박스를 사용하여 할일의 상태를 변경할 수 있습니다. 체크된 할일은 별도의 '완료된 할일' 탭에 분류하여 관리할 수 있도록 했으며, 상태 변화에 따라 UI가 동적으로 업데이트되도록 구현하였습니다. <br /><br />
